...
export const updateTask = async(request: Request, response: Response) => {
    ...
    if (task.affected == 1){
        const taskUpdated = await getRepository(Tasks).findOne({where: {id: parseInt(id, 10)}})
...
export const finishedTask = async(request: Request, response: Response) => {
    ...
    if (task.affected == 1){
        const taskFinished = await getRepository(Tasks).findOne({where: {id: parseInt(id, 10)}})
